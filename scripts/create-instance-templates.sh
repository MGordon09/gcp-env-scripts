...
# make ngs-tools VM instance template
...
create_ngs_tools(){
    gcloud compute instance-templates create mhra-ngs-tools-v3 \
        ...
        --network-interface=subnet=projects/$host_prj/regions/europe-west2/subnetworks/$prj_prefix-eu-west2-1,no-address \
        --network-interface=subnet=$prj_prefix-eu-west2-1,no-address \
        --metadata=enable-oslogin=true \
        --maintenance-policy=MIGRATE \
        --provisioning-model=STANDARD \
        --service-account=ngs-tools@$project_name.iam.gserviceaccount.com \
